...
def create_app(config_class=Config, use_talisman=True):
    ...
    app.blast_queue = rq.Queue('blast', connection=app.redis, default_timeout=1*60*60)
    app.process_blasts_queue = rq.Queue('process_blasts', connection=app.redis, default_timeout=2*60*60)
    app.alignment_queue = rq.Queue('alignment', connection=app.redis, default_timeout=12 * 60 * 60)
