...
def execute_mongo_dump():
    ...
    if current_app.config['MONGODB_HOST'] == 'localhost':
        command = f'''mongodump --uri="mongodb://localhost:{current_app.config['MONGODB_PORT']}" --db={current_app.config['MONGODB_DB']} --gzip --archive={output_path} --verbose'''
    else:
        command = f'''mongodump --uri="{current_app.config['MONGODB_HOST']}:{current_app.config['MONGODB_PORT']}" --db={current_app.config['MONGODB_DB']} --gzip --archive={output_path} --verbose'''
    print(f"CMD = {command}")
